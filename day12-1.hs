...
cat day12-1.test | ./day12-1
10

cat day12-2.test | ./day12-1
19

cat day12-3.test | ./day12-1
226
...
allRoutes :: [Path] -> [[Cave]]
allRoutes paths = filter ((==end) . last) $ (start:) <$> go S.empty start
  where
    ...
    go visited from = pure [] <> do             -- accumulate all paths
      ...
      (to :) <$> go (S.insert to visited) to    -- continue with the next cave
